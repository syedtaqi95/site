...
Many databases internally use a _log_, which is an append-only data file. Real
...
In order to efficiently look up records in a database, we need an _index_. An
...
Key-value stores are quite similar to _dictionary_ types, which are usually 
...
segment file. Apply _compaction_ and _merging_ on old segments to reduce disk usage
...
binary format.
- Deleting records: Append a _tombstone_ (deletion record) to delete keys, which
...
key-value pairs appear in sequential write order, the key-value pairs are _sorted 
by key_, and each key only appears once within each merged segment file.
...
available memory. This is similar to the _mergesort_ algorithm.
...
inspired by Google's Bigtable paper (which introduced the terms _memtable_ and 
_SSTable_).
...
uses a similar method for storing its _term dictionary_. A full-text index is much
...
database. For this scenario, _Bloom filters_ are used. A **Bloom filter** is a 
...
merging and compaction. The most common options are _size-tiered_ and _leveled_
...
B-trees break the database down into fixed-size _blocks_ or _pages_, traditionally
...
The final page containing the individual keys is called a _leaf page_.

The number of references to child pages in one page is called the _branching 
factor_. It depends on the amount of space required to store the page references
...
In order to make this reliable, B-tree implementations include a _write-ahead log_
(WAL, or _redo log_). This is an append-only log to which every B-tree modification
...
_latches_ (lightweight locks). Log-structured indexes are simpler for this, as
...
parent pages.
- B-tree variants like _fractal trees_ borrow some log-structured ideas to reduce
disk seeks.
